...
// All owned domains + thenycmobilesalon.com — referrals from these are internal, not real traffic
const OWNED_HOSTS = new Set([
  ...
  'thenycmobilesalonservice.com',
  'www.thenycmobilesalonservice.com',
])
...
export function isAdminReferrer(referrer: string | null | undefined): boolean {
  if (!referrer) return false
  return (
    referrer.includes('thenycmobilesalon.com/admin') ||
    referrer.includes('thenycmobilesalon.com/team') ||
    referrer.includes('thenycmobilesalon.com/book/dashboard') ||
