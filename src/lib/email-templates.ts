...
export function clientConfirmationEmail(booking: any) {
  ...
  const content = `
    ...
    ${primaryButton('View Your Portal', 'https://www.thenycmobilesalon.com/book')}

    <p style="color: #666; font-size: 14px; margin: 16px 0 0 0;">
      You can update your notes and view upcoming bookings anytime at <a href="https://www.thenycmobilesalon.com/book" style="color: #000; font-weight: 500;">thenycmobilesalon.com/book</a>
    </p>
...
export function clientPaymentDueEmail(booking: any, amount: string) {
  ...
  const content = `
    ...
    ${infoTable(`
      ${infoRow('Zelle', '<a href="mailto:hi@thenycmobilesalon.com" style="color: #0066cc;">hi@thenycmobilesalon.com</a>')}
...
export function referralWelcomeEmail(referrer: { name: string; ref_code: string; preferred_payout: string }) {
  ...
  const content = `
    ...
    ${primaryButton('View Your Dashboard', `https://www.thenycmobilesalon.com/referral-dashboard?code=${referrer.ref_code}`)}
  ...
  return { subject: `Welcome to The NYC Mobile Salon, ${firstName}!`, html: emailWrapper(content) }
}
...
export function newReferrerAdminEmail(referrer: { name: string; email: string; phone?: string; ref_code: string; preferred_payout: string; zelle_email?: string }) {
  const content = `
    ...
    ${primaryButton('View Referrals', 'https://www.thenycmobilesalon.com/admin/referrals')}
  `
...
export function cleanerWelcomeEmail(cleaner: { name: string; pin: string; phone: string }) {
  ...
  const content = `
    ...
    <div style="background: #fff8e6; border: 1px solid #f0e6cc; border-radius: 8px; padding: 20px; margin: 32px 0; text-align: left;">
      ...
      <p style="font-size: 13px; color: #555; margin: 0;">Ve a: <strong>thenycmobilesalon.com/team</strong><br>Ingresa tu número de teléfono para ver tus trabajos.</p>

      <h3 style="font-size: 14px; color: #333; margin: 16px 0 8px 0;">📲 GUARDAR EN TU TELÉFONO</h3>
      <p style="font-size: 13px; color: #555; margin: 0 0 8px 0;"><strong>iPhone:</strong> Abre thenycmobilesalon.com/team en Safari → Toca compartir (↑) → "Añadir a pantalla de inicio"</p>
...
export function adminDailyNotificationDigestEmail(data: {
  ...
  const content = `
    ...
    ${primaryButton('View Dashboard', 'https://www.thenycmobilesalon.com/admin/bookings')}
  `
...
export function adminPendingRemindersEmail(pendingBookings: { client_name: string; date: string; service_type: string }[]) {
  ...
  const content = `
    ...
    ${primaryButton('Review Pending Bookings', 'https://www.thenycmobilesalon.com/admin/bookings')}
  `
...
export function adminDailyOpsRecapEmail(data: {
  ...
  const content = `
    ...
    ${primaryButton('Open Dashboard', 'https://www.thenycmobilesalon.com/admin/bookings')}
  `
