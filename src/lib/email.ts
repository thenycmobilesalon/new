...
const ADMIN_DOMAINS = ['thenycmobilesalon.com', 'thenycmobilesalon.gmail.com']

function isAdminEmail(email: string): boolean {
  return ADMIN_DOMAINS.some(d => email.toLowerCase().endsWith(`@${d}`) || email.toLowerCase().includes('thenycmobilesalon'))
}
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      const { data, error } = await resend.emails.send({
        from: 'The NYC Mobile Salon <hi@thenycmobilesalon.com>',
        to,
        subject,
        html,
        attachments,
